...
MAXLIST = "https://github.com/fatyzzz/max-list/releases/latest/download"
...
GEOSITE = OrderedDict([
    ...
    ("IP-CHECK", {
        "urls": [f"{MAXLIST}/ip-check-formatted.txt"],
        "extra": [],
    }),
    ("VPNDETECT", {
        "urls": [f"{MAXLIST}/vpndetect.txt"],
        "extra": [],
    }),
    ("ESCAPEFROMTARKOV", {
...
def _load_maxlist_domains():
    """Download ip-check and vpndetect domains from fatyzzz/max-list."""
    domains = []
    for fname in ("ip-check-formatted.txt", "vpndetect.txt"):
        text = download(f"{MAXLIST}/{fname}")
        if text:
...
def generate_v2raytun_config():
    ...
    # Inject max-list domains into V2RayTUN routing
    maxlist_domains = _load_maxlist_domains()
    if maxlist_domains:
        # Find the direct-ru-services rule and add max-list domains
        for rule in V2RAYTUN_ROUTING["rules"]:
            if rule.get("id") == _v2raytun_uuid("direct-ru-services"):
                rule["domain"] = rule["domain"] + maxlist_domains
                print(f"  + max-list: {len(maxlist_domains)} domains → direct-ru-services")
                break
